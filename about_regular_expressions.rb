...
class AboutRegularExpressions < Neo::Koan
  def test_a_pattern_is_a_regular_expression
    assert_equal Regexp, /pattern/.class        #正则表达式
  end
  ...
  def test_shortcut_character_classes_are_negated_with_capitals
    assert_equal "the number is ", "the number is 42"[/\D+/]   #A non-digit character ([^0-9])，没有数字
    assert_equal "space:", "space: \t\n"[/\S+/]    #A non-whitespace character: /[^ \t\r\n\f]/，没有空格
    # ... a programmer would most likely do
    assert_equal " = ", "variable_1 = 42"[/[^a-zA-Z0-9_]+/]  #没有字母和数字
    assert_equal " = ", "variable_1 = 42"[/\W+/]         #没有字母和数字
  end

  # ------------------------------------------------------------------

  def test_slash_a_anchors_to_the_start_of_the_string
    assert_equal 'start', "start end"[/\Astart/]  # \A - 匹配字符串的开始。
    assert_equal nil, "start end"[/\Aend/]
  end

  def test_slash_z_anchors_to_the_end_of_the_string
    assert_equal "end", "start end"[/end\z/]  #  \Z - 匹配字符串的结尾
    assert_equal nil, "start end"[/start\z/]
  end

  def test_caret_anchors_to_the_start_of_lines
    assert_equal "2", "num 42\n2 lines"[/^\d+/]   #匹配转折点之后的开头
  end
  ...
  def test_parentheses_group_contents
    assert_equal "hahaha", "ahahaha"[/(ha)+/]    #匹配 ha 的多个值
  end
